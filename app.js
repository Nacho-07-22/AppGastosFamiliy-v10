...
async function initFirebase() {
  ...
  if (firebaseEnabled && db && auth.currentUser) {
    // Solo mostrar usuarios de la nube si hay sesión activa
    db.collection("users")
      .limit(50)
      .get()
      .then((snapshot) => {
      ...
      .catch(() => {
        ul.innerHTML =
          '<li style="color:red">Error al cargar usuarios de la nube</li>';
      });
  } else {
    // Solo usuarios locales (si no hay sesión activa en Firebase)
    cargarUsuarios();
    usuarios.forEach((u) => {
      const li = document.createElement("li");
      li.innerHTML = `<i class=\"fa-solid fa-user\"></i> ${escapeHtml(
  ...
  loginForm.addEventListener("submit", async (e) => {
    ...
    if (firebaseEnabled) {
      ...
      if (!auth || !db) {
        const initiated = await initFirebase();
        if (!initiated) {
          loginError.textContent =
            "Error al conectar con la base de datos. Intenta de nuevo.";
          return;
        }
      }
    ...
    if (
      !confirm(
        `¿Seguro que deseas eliminar el gasto '${
          gasto.desc
        }' de ₡${formatColones(gasto.monto)}? Esta acción no se puede deshacer.`
      )
    )
      return;
